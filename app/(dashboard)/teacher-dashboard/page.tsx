  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900">{t.teacherDashboard.title}</h1>
          <p className="text-slate-500 font-medium mt-1">{t.teacherDashboard.subtitle}</p>
        </div>
        <button onClick={() => router.push("/create")} className="bg-white border-2 border-orange-100 text-orange-600 px-6 py-3 rounded-xl font-bold hover:bg-orange-50 transition flex items-center gap-2">
          <Plus className="w-5 h-5" /> {t.teacherDashboard.createMaterial}
        </button>
      </header>
      ...
      <div className="mb-12">
        <h2 className="text-xl font-black text-slate-900 mb-6 flex items-center gap-2">
          <Award className="w-5 h-5 text-orange-600" /> {t.teacherDashboard.myLibrary}
        </h2>
          ...
          {decks.map((deck) => (
            <motion.div key={deck.id} whileHover={{ y: -4 }} className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm hover:shadow-lg transition-all">
              <div className="flex justify-between items-start mb-4">
                <div>
                   <span className={clsx("text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wide flex w-fit items-center gap-1 mb-2", deck.content_type === "quiz" ? "bg-purple-100 text-purple-700" : "bg-orange-100 text-orange-700")}>
                      ...
                      {deck.content_type === "quiz" ? t.library.quiz : t.library.flashcards}
                   </span>
              ...
              <div className="flex items-center justify-between text-sm text-slate-500 mb-6 font-medium">
                 <span>{deck.total_cards} {t.teacherDashboard.cards}</span>
              </div>
              <button
              ...
              >
                <MonitorPlay className="w-5 h-5" /> {t.teacherDashboard.startSession}
              </button>
            </motion.div>
          ))}
        </div>
      ...
      <div>
        <h2 className="text-xl font-black text-slate-900 mb-6 flex items-center gap-2">
          <Clock className="w-5 h-5 text-orange-600" /> {t.teacherDashboard.sessionHistory}
        </h2>
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                <th className="text-left py-4 px-6 text-xs font-bold text-slate-500 uppercase">{t.teacherDashboard.date}</th>
                <th className="text-left py-4 px-6 text-xs font-bold text-slate-500 uppercase">{t.teacherDashboard.material}</th>
                <th className="text-left py-4 px-6 text-xs font-bold text-slate-500 uppercase">{t.teacherDashboard.participants}</th>
                <th className="text-left py-4 px-6 text-xs font-bold text-slate-500 uppercase">{t.teacherDashboard.status}</th>
              </tr>
            </thead>
            <tbody>
              {sessions.map((s) => (
                <tr key={s.id} className="border-b border-slate-100 hover:bg-slate-50 transition">
                  ...
                  <td className="py-4 px-6">
                    <span className={clsx("px-2 py-1 rounded-md text-xs font-bold uppercase", s.status === "completed" ? "bg-slate-100 text-slate-500" : "bg-green-100 text-green-700 animate-pulse")}>
                      {s.status === "completed" ? t.teacherDashboard.completed : t.teacherDashboard.ongoing}
                    </span>
                  </td>
                </tr>
              ))}
              {sessions.length === 0 && (
                <tr><td colSpan={4} className="py-8 text-center text-slate-400 font-medium">{t.teacherDashboard.historyEmpty}</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        {showStartModal && selectedDeckForSession && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white rounded-3xl p-8 max-w-md w-full">
              <h3 className="text-2xl font-black text-slate-900 mb-2">{t.teacherDashboard.sessionSetup}</h3>
              <p className="text-slate-500 mb-6">{selectedDeckForSession.name}</p>

              <div className="mb-6">
                <label className="block text-sm font-bold text-slate-700 mb-2">{t.teacherDashboard.participantsLimit}</label>
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowStartModal(false)} className="flex-1 bg-slate-100 text-slate-700 py-3 rounded-xl font-bold">{t.teacherDashboard.cancel}</button>
                <button onClick={handleStartSession} disabled={startingSession} className="flex-1 bg-orange-600 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2">
                  {startingSession ? <Loader2 className="animate-spin w-5 h-5" /> : <Play className="w-5 h-5" />} {t.teacherDashboard.start}
                </button>
              </div>
            </motion.div>
          </motion.div>

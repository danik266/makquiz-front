...
export default function TeacherDashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      const decksRes = await fetch("http://127.0.0.1:8000/api/decks/my?limit=50", { headers });
      if (decksRes.ok) setDecks(await decksRes.json());
      
      // Загружаем историю сессий
      const historyRes = await fetch("http://127.0.0.1:8000/api/live/history", { headers });
  ...
  const handleStartSession = async () => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/live/create", {
        method: "POST",
        headers: {

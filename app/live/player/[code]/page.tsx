...
export default function PlayerPage() {
  ...
  const joinSession = async () => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/live/join", {
  ...
  useEffect(() => {
    ...
    fetch(`http://127.0.0.1:8000/api/live/${sessionId}/cards`)
      .then(r => r.ok ? r.json() : [])
      .then(data => {
          setCards(data);
    ...
    const interval = setInterval(async () => {
      ...
      try {
        const res = await fetch(`http://127.0.0.1:8000/api/live/${sessionId}/status`);
        if (res.ok) {
            const data = await res.json();
  ...
  const submitAnswer = async () => {
    ...
    try {
        const res = await fetch(`http://127.0.0.1:8000/api/live/${sessionId}/answer`, {
            method: "POST", 
